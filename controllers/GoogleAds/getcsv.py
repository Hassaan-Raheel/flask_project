...
UPLOAD_FOLDER = 'uploads'  # Directory where data.csv is stored
...
def get_cleaned_csv_data():
    try:
        ...
        # Read the CSV while skipping the first row (if it's metadata)
        df = pd.read_csv(CSV_FILE, skiprows=1)  

        # Drop completely empty columns
        df = df.dropna(axis=1, how='all')

        # Rename columns using the second row and drop it
        df.columns = df.iloc[0]  # Assign second row as header
